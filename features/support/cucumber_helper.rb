# Dir[Rails.root.join('spec', 'spec_helper.rb')].sort.each { |f| require f }
...
def create_my_player(board_height: 10, board_width: 10)
  ...
  @my_player.create_board(cells: Board.generate(height: board_height, width: board_width))
end
...
def create_opponent_player(board_height: 10, board_width: 10)
  ...
  @opponent_player.create_board(cells: Board.generate(height: board_height, width: board_width))
end
